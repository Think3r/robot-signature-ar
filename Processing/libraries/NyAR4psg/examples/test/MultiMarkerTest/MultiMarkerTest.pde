...
void draw() {
  ...
  if(nya.isExistMarker(0)){
    nya.beginTransform(0);//マーカ座標系に設定
    {
      rotateY(-angoloX);
      rotateX(-angoloY+PI);
      // angolo X e Y dipendono da dove clicchiamo con il mouse
      rotateX(-PI/2);
      rotateZ(-PI/2);
      /* abbiamo un nuovo sistema di riferimento destro dove l'asse x è rimasto immutato
      l'asse y è il vecchio asse z e l'asse z è il vecchio asse y*/
      pushMatrix();
...
void braccioDH(float thetaA, float dA, float alphaA, float aA){
  noStroke();   
  braccio(dA, 1);
  rotateY(thetaA); //sarebbe Z nel sistema destrorso
  braccio(aA, 0); 
  rotateX(alphaA);
}
/*
void braccioDH(float thetaA, float dA, float alphaA, float aA){
  ...
  translate(0, dA, 0); //traslo lungo y, immaginando che sia la mia z
...
float [][] selectDH(int robot){
  ...
  { 
    ...
    float [][] tabellaDHp={{0,30*q1,-PI/2,0},{-PI/2,30*q2,-PI/2,0},{0,30*q3,0,0}/*,{30*q4,0,-PI/2,0},{30*q5,0,PI/2,0},{30*q6,L6,0,0}*/};
    tabDH = tabellaDHp;
    q1=2; q2=4; q3=4; q4=0; q5=0; q6=0;
    break;
    case 3: // "Robot Cilindrico"
    float [][] tabellaDHc={{q1,D1,0,0},{0,30*q2,-PI/2,0},{0,30*q3,0,0}/*,{30*q4,0,-PI/2,0},{30*q5,0,PI/2,0},{30*q6,L6,0,0}*/};
    tabDH = tabellaDHc;
    ...
    float [][] tabellaDHt={{q1,D1,PI/2,0},{q2,0,PI/2,L2},{0,30*q3,0,0}/*,{30*q4,0,-PI/2,0},{30*q5,0,PI/2,0},{30*q6,L6,0,0}*/};
    tabDH = tabellaDHt;
    q1=2; q2=0; q3=3; q4=0; q5=0; q6=0;
    break;
    case 6: // "Robot Sferico (tipo Stanford)"
    float [][]tabellaDHf={{q1,D1,-PI/2,0},{q2,D2,PI/2,0},{0,30*q3,0,0}};
    tabDH = tabellaDHf;
